...
export function WorkspaceSwitcher() {
    ...
    const migratedWorkspaces = workspaces.filter(
        (workspace) => workspace.is_migrated_source === true,
    );
                            ...
                            <SidebarMenuButton
                                ...
                                <div className="ml-1 grid flex-1 text-left text-sm leading-tight">
                                    ...
                                    <span className="truncate text-xs text-muted-foreground capitalize">
                                        {currentWorkspace.is_migrated_source
                                            ? t(
                                                  'workspace_switcher.read_only_workspace',
                                                  'Read-only migrated source',
                                              )
                        ...
                        >
                            <div className="p-1">
                                ...
                                {migratedWorkspaces.length > 0 ? (
                                    <DropdownMenuSub>
                                        <DropdownMenuSubTrigger className="cursor-pointer rounded-md px-2 py-2 text-muted-foreground">
                                            {t(
                                                'workspace_switcher.migrated_workspaces',
                                                'Migrated workspaces',
                                            )}
                                        </DropdownMenuSubTrigger>
                                        <DropdownMenuSubContent className="min-w-64">
                                            ...
                                            {migratedWorkspaces.map((workspace) => (
                                                <DropdownMenuItem
                                                    key={workspace.id}
                                                    onClick={() => switchWorkspace(workspace.id)}
                                                    className="cursor-pointer rounded-md px-2 py-2"
