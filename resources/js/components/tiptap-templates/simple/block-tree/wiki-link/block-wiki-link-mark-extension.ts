...
export const BlockWikiLinkMark = Mark.create<{
    ...
    addAttributes() {
        return {
            ...
            crossWorkspace: {
                default: false,
                parseHTML: (element: HTMLElement) =>
                    element.getAttribute('data-cross-workspace') === 'true',
                renderHTML: (attributes: BlockWikiLinkAttributes) => ({
                    'data-cross-workspace': attributes.crossWorkspace ? 'true' : null,
                }),
            },
        };
    },
    ...
    renderHTML({ HTMLAttributes }) {
        const isCrossWorkspace = String(HTMLAttributes['data-cross-workspace'] ?? '') === 'true';

        return [
            'span',
            mergeAttributes(HTMLAttributes, {
                'data-wikilink': 'true',
                class: isCrossWorkspace ? 'md-wikilink md-wikilink-cross-workspace' : 'md-wikilink',
                spellcheck: 'false',
            }),
            0,
        ];
    ...
    addProseMirrorPlugins() {
        const buildNotesByTargetPath = () => {
            ...
            for (const note of this.options.notes) {
                ...
                notesById.set(note.id, mapped);
            }

            return { notesByTargetPath, notesById };
        };
        ...
        const openWikiLinkForEditing = (
            ...
            const { notesByTargetPath, notesById } = buildNotesByTargetPath();
            const currentText = view.state.doc.textBetween(range.from, range.to);
            let targetPath = String(
                mark.attrs.targetPath || mark.attrs.noteId || currentText,
            ).trim();
            const markNoteId =
                typeof mark.attrs.noteId === 'string' ? mark.attrs.noteId.trim() : '';
            const looksLikeUuid =
                /^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(
                    targetPath,
                );
            if (looksLikeUuid && markNoteId !== '') {
                const resolvedById = notesById.get(markNoteId);
                if (resolvedById) {
                    targetPath = resolvedById.targetPath;
                }
            }
            const resolvedById = markNoteId !== '' ? notesById.get(markNoteId) : null;
            const targetBlockId =
                typeof mark.attrs.targetBlockId === 'string'
                    ? mark.attrs.targetBlockId.trim()
                    : '';
            const resolved =
                resolvedById ||
                notesByTargetPath.get(targetPath.toLowerCase());
            const headingTitle =
            ...
            new Plugin({
                ...
                appendTransaction: (transactions, _oldState, newState) => {
                    ...
                    newState.doc.descendants((node, pos) => {
                        ...
                        const noteId = String(wikiMark.attrs.noteId ?? '').trim();
                        const resolved =
                            (noteId !== '' ? notesById.get(noteId) : null) ||
                            notesByTargetPath.get(targetPath.toLowerCase());
                        if (!resolved) {
                            return true;
                        }
                        ...
                        tr.addMark(
                            ...
                            markType.create({
                                ...
                                targetPath: resolved.targetPath,
                                targetBlockId: targetBlockId || null,
                                crossWorkspace: resolved.isCrossWorkspace,
                            }),
                        );
                        changed = true;
                        ...
                        sorted.forEach((pending) => {
                            ...
                            tr.addMark(
                                ...
                                markType.create({
                                    ...
                                    targetBlockId,
                                    crossWorkspace: resolved?.isCrossWorkspace ?? false,
                                }),
                            );
                            changed = true;
                        });
